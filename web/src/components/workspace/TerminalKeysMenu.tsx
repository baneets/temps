// Floating dropdown that injects special keys into the active terminal.
//
...
// blurs it would dismiss the keyboard, so every key here uses onMouseDown
// + preventDefault and refocuses the terminal after sending. The trigger
// sits at the bottom-right of the terminal pane (anchored from the parent)
// — when tapped it opens upward as a small grid of common keys missing
// from mobile soft keyboards (Tab, Shift+Tab, Esc, arrows, Ctrl combos).
...
interface TerminalKeysMenuProps {
  ...
  terminalRef: React.RefObject<SessionTerminalHandle | null>
}
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        ...
        align="end"
        sideOffset={6}
        className="w-[min(20rem,calc(100vw-1rem))] max-h-[60vh] overflow-y-auto border-white/10 bg-[#0b0b0f] p-2"
