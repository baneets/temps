...
/**
 * Immediately enqueue a run for the given schedule (Run Now).
 * Reuses the exact same `EnqueueJobParams` path as the cron scheduler.
 * Returns `202 Accepted` with the new `BackupResponse`. Returns `409 Conflict`
 * if a backup for the same engine + target is already pending or running, or
...
/**
 * Paginated run history for a backup schedule.
 * Returns one entry per `backups` row linked to this schedule, enriched with
 * the most-recent `backup_jobs` step and attempt count (single DB round-trip).
