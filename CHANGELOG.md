...
- **Compose `volumes_from` and absolute bind-mount closure**: the compose security policy now rejects `volumes_from` outright (in the base compose, inline overrides, and its interpolation guard), closing a bypass where `volumes_from: container:<name>` could inherit every volume of an arbitrary host container — other tenants' or Temps infrastructure containers — without naming a path. Absolute host bind sources are now rejected unconditionally: the previous `/tmp/` exemption is removed, since `/tmp` is world-writable and can hold another project's data-dir artifacts (`.env.temps`, encryption keys) when the data directory lives under it.
- **Compose validator gap closure (review follow-ups)**: extended the security policy to reject additional host-affecting directives found in review — `sysctls`, `group_add`, and `cgroup_parent` in the base compose (previously enforced only on inline overrides); `deploy.resources.reservations.devices` (the long form of the already-blocked `gpus:` accelerator passthrough); `build.context`/`build.dockerfile` pointing at absolute or project-escaping host paths, and `build.ssh` agent forwarding; and named volumes using a non-`local` driver or a network filesystem `type` (`nfs`/`cifs`/…). Service names are now validated against the Compose character set so they cannot corrupt the generated `docker-compose.temps-security.yml` override.
...
- **Inline compose override allow-list**: inline overrides supplied via project settings are now validated structurally before being handed to the host Docker daemon — they may only modify services that already exist in the repository compose file, may not introduce top-level keys other than `services`, and may not use host-affecting service keys (`privileged`, `network_mode`, `pid`/`ipc`/`uts`/`cgroup`/`userns_mode`, `cap_add`/`cap_drop`, `devices`, `device_cgroup_rules`, `security_opt`, `sysctls`, `volumes`). This complements the value-level security policy as defense-in-depth.
