...
impl WorkspaceSessionResponse {
    /// Build a response without the preview password populated. Used when
    /// the caller doesn't have access to the encryption service or has
    /// already decided not to expose the plaintext (e.g. in tests). Most
    /// production handlers should use [`from_model_with_password`] instead.
    pub fn from_model(s: workspace_sessions::Model, parts: &PreviewUrlParts) -> Self {
        Self::from_model_with_password(s, parts, None)
    }

    /// Build a response with an explicit decrypted preview password. The
    /// caller passes `Some(plaintext)` after asking the workspace service
    /// to decrypt the row's `preview_password_encrypted`. Sessions whose
    /// encrypted column is not set (legacy rows from before reversible
    /// storage was added) get `None` here and the UI tells the user to
    /// regenerate to view it.
    pub fn from_model_with_password(
        s: workspace_sessions::Model,
        parts: &PreviewUrlParts,
        preview_password: Option<String>,
    ) -> Self {
        let id = s.id;
...
pub async fn workspace_regenerate_preview_password(
    ...
    let preview_parts = preview_url_parts(&app_state).await;
    let mut resp =
        SessionResponse::from_model_with_password(session, &preview_parts, Some(plaintext.clone()));
...
pub async fn workspace_list_sessions(
    ...
    Ok(Json(SessionListResponse {
        sessions: sessions
            .into_iter()
            .map(|s| {
                let pwd = app_state.workspace_service.decrypt_preview_password(&s);
                SessionResponse::from_model_with_password(s, &preview_parts, pwd)
            })
            .collect(),
        total,
        page,
...
pub async fn workspace_get_session(
    ...
    let pwd = app_state
        .workspace_service
        .decrypt_preview_password(&swm.session);
    Ok(Json(SessionWithMessagesResponse {
        session: SessionResponse::from_model_with_password(swm.session, &preview_parts, pwd),
        messages: swm
            .messages
            .into_iter()
...
pub async fn workspace_update_session(
    ...
    let pwd = app_state
        .workspace_service
        .decrypt_preview_password(&updated);
    Ok(Json(SessionResponse::from_model_with_password(
        updated,
        &preview_parts,
        pwd,
    )))
...
pub async fn workspace_reopen_session(
    ...
    let pwd = app_state
        .workspace_service
        .decrypt_preview_password(&session);
    Ok((
        StatusCode::OK,
        Json(SessionResponse::from_model_with_password(
            session,
            &preview_parts,
            pwd,
        )),
