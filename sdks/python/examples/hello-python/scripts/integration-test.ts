/**
 * Integration test for the Hello Python Plugin compiled binary.
 *
 ...
 *
 * Tests the same contract that the Temps host expects:
...
const BINARY = join(import.meta.dirname, "..", "dist", "hello-python");
const SOCKET_PATH = "/tmp/tp-test-python-plugin/hello-python.sock";
const DATA_DIR = "/tmp/tp-test-python-plugin-data";
...
async function main(): Promise<void> {
  cleanup();
  mkdirSync("/tmp/tp-test-python-plugin", { recursive: true });
  ...
  console.log("\n=== Integration Test: Python Plugin Binary ===\n");
  ...
  const readLine = async (): Promise<string> => {
    ...
    return new Promise<string>((resolve, reject) => {
      const timeout = setTimeout(
        () =>
          reject(
            new Error("Handshake timeout (no stdout line within 15s)")
          ),
        15_000
      );
  ...
  assertEqual(manifestMsg.name, "hello-python", "Plugin name");
  assertEqual(manifestMsg.version, "0.1.0", "Plugin version");
  assertEqual(manifestMsg.display_name, "Hello Python", "Display name");
  ...
  // Wait for the Unix socket to be fully ready
  await sleep(1000);
  ...
  assertEqual(
    ...
    "Hello includes user email"
  );
  assertEqual(
    helloBody.plugin,
    "hello-python",
    "Hello includes plugin name"
  );
  assert(
    helloBody.dataDir.includes("tp-test-python-plugin-data"),
    "Hello includes data dir"
  );

  // Test /projects endpoint -- the WebSocket channel is connected but no host
  // responds to requests, so we simulate a host response via the WS connection.
  // The plugin sends a list_projects request; we respond with an empty list.
  const projectsPromise = httpRequest(SOCKET_PATH, "/projects");

  // Wait a moment for the request to arrive at the plugin, which will
  // send a WebSocket message to our connected ws. We listen and reply.
  const wsResponsePromise = new Promise<void>((resolve) => {
  ...
  assertEqual(projectsRes.status, 200, "Projects returns 200");
  ...
  // aiohttp may return 301 or 302 for redirects
  assert(
    uiRedirectRes.status === 301 ||
      uiRedirectRes.status === 302,
    `GET /ui redirects (${uiRedirectRes.status})`
  );
  assertEqual(
    uiRedirectRes.headers.get("location"),
    "/ui/",
    "Redirects to /ui/"
  );
  ...
  assert(
    uiIndexRes.headers.get("content-type")?.includes("text/html") ??
      false,
    "Index has text/html content type"
  );
  assert(
    uiIndexRes.body.includes("<!DOCTYPE html>"),
    "Index contains HTML"
  );
  assert(
    uiIndexRes.body.includes("root"),
    "Index contains root element"
  );
  assert(
    uiIndexRes.body.includes("Hello Python Plugin"),
    "Index has correct title"
  );
  assertEqual(
  ...
  const jsMatch = uiIndexRes.body.match(
  ...
  if (jsMatch) {
    ...
    assert(
      jsRes.body.length > 1000,
      "JS asset has substantial content"
    );
  } else {
    console.error(
      "  FAIL: Could not find JS asset reference in index.html"
    );
    failed++;
  }
  ...
  assert(
    spaRes.body.includes("<!DOCTYPE html>"),
    "SPA fallback serves index.html"
  );

  // Non-existent asset file should 404
  const missing404 = await httpRequest(
    SOCKET_PATH,
    "/ui/assets/nonexistent.js"
  );
  assertEqual(missing404.status, 404, "Missing asset returns 404");

  // Path traversal should be blocked (403) or normalized away (404)
  // HTTP clients may normalize /../ before sending, so either status is acceptable
  ...
  assert(
    ...
    `Path traversal blocked (${traversalRes.status})`
  );
  ...
  // Wait for exit (Nuitka onefile needs more time for cleanup)
  const exitCode = await Promise.race([
    child!.exited,
    sleep(10000).then(() => -1),
  ]);

  assert(exitCode !== -1, "Plugin exited within 10 seconds");
  await sleep(500); // Let stderr reader finish
  const hasShutdownLog = stderrLines.some((l) =>
    l.includes("Shutting down") || l.includes("shutting down")
  );
