...
<Note>
  Next.js, Nuxt, Remix, SvelteKit, and NestJS are always deployed as server containers, even when they support static export. If you want pure static hosting for a Next.js project with `output: 'export'`, use Vite or add your own Dockerfile with nginx.
</Note>
...
This works out of the box with no configuration. You do not need to add `_redirects`, `vercel.json`, or nginx rewrite rules.
